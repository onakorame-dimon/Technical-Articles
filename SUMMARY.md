# Table of contents

## 👋 WELCOME!

* [whoami](README.md)
* [Technical-Articles](welcome/readme.md)

## AZURE

* [Identity-basics-Azure](azure/identity-basics-azure.md)
* [Azure Load Balancer Floating IP](azure/azure-lb-floating-ip.md)

## AWS

* [Creating an IAM user](aws/creating-an-iam-user.md)
* [securing-AWS-root-user](aws/securing-aws-root-user.md)
* [IAM-basics-AWS](aws/iam-basics-aws.md)

## Iac 

* [Markdown cheatsheet](Iac/Markdown%20cheatsheet.md)
* [Vagrant cheatsheet](Iac/Vagrant%20cheatsheet.md)

## NETWORKING

* [APIPA (Automatic Private IP Addressing)](networking/apipa.md)

## CONTAINERIZATION

* [Page 6](containerization/page-6.md)

## LINUX

* [System Boot up Process](linux/boot-up-process.md)
* [BASH scripting syntax guide](linux/bash-guide.md)
* [Regex Cheatsheet](linux/regex-cheatsheet.md)

## PROJECTS

* [BASH Multiplication Table](projects/bash-multiplication-table.md)
